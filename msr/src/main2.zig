...
const session_server2 = @import("session_server2");
    ...
    switch (bridge_exit) {
        ...
        .remote_closed => {
            if (c.isatty(c.STDERR_FILENO) == 1) {
                err("msr2: session closed the current attachment\n", .{});
            }
        },
        .stdout_unavailable => {
            if (c.isatty(c.STDERR_FILENO) == 1) {
                err("msr2: local terminal output became unavailable; session is still running\n", .{});
            }
        },
        .remote_error => {
            err("msr2: attach stream failed\n", .{});
            return 1;
        },
    }
...
fn runAttachNested(current_session: []const u8, target: []const u8, force_requested: bool) u8 {
    if (force_requested) {
        err("msr2: nested attach does not support -f|--force; use direct attach for ownership takeover\n", .{});
        return 1;
    }
    if (std.mem.eql(u8, current_session, target)) {
        err("msr2: cannot attach current session to itself\n", .{});
        return 1;
    }

    ownerForwardWithRetry(current_session, .{ .attach = @constCast(target) }) catch |e| {
        err("msr2: nested attach passthrough failed: {s}\n", .{@errorName(e)});
        return 1;
    };
    return 0;
}
...
fn runDetachNested(current_session: []const u8) u8 {
    ownerForwardWithRetry(current_session, .detach) catch |e| {
        err("msr2: nested detach passthrough failed: {s}\n", .{@errorName(e)});
        return 1;
    };
    return 0;
}
...
fn runHost(path: []const u8, child_argv: []const []const u8) !u8 {
    ...
    session_server.listen(path) catch |e| {
        switch (e) {
            session_server2.Error.AlreadyExists => err("msr2: session already exists at {s}\n", .{path}),
            session_server2.Error.PermissionDenied => err("msr2: permission denied creating session socket at {s}\n", .{path}),
            session_server2.Error.PathTooLong => err("msr2: session socket path is too long: {s}\n", .{path}),
            else => err("msr2: failed to create session at {s}\n", .{path}),
        }
        return 1;
    };

    while (true) {
        const progressed = session_server.step() catch |e| {
            err("msr2: internal server step failed: {s}\n", .{@errorName(e)});
...
pub fn main(init: std.process.Init) !u8 {
    ...
    if (argv.len >= 2 and std.mem.eql(u8, argv[1], "_host")) {
        if (argv.len < 4) {
            err("msr2: invalid _host arguments\n", .{});
            return 1;
        }
                ...
                .unknown_command => {
                    ...
                    if (failure.command) |cmd_kind| {
                        switch (cmd_kind) {
                            ...
                            .current => {
                                err("msr2: current does not take additional arguments\n", .{});
                                usageForCommandKind(.current);
                            },
                            .create => {
                                err("msr2: invalid create arguments\n", .{});
                                usageForCommandKind(.create);
                            },
                            .attach => {
                                if (raw_current_session != null) {
                                    err("msr2: nested attach requires <target> and does not support -f|--force\n", .{});
                                    usageAttachNested();
                                } else {
                                    err("msr2: attach requires <path> and optional -f|--force\n", .{});
                                    usageForCommandKind(.attach);
                                }
                            },
                            .detach => {
                                err("msr2: detach does not take additional arguments\n", .{});
                                usageForCommandKind(.detach);
                            },
                            .resize => {
                                err("msr2: resize requires <path> <cols> <rows> and optional -f|--force\n", .{});
                                usageForCommandKind(.resize);
                            },
                            .terminate => {
                                err("msr2: terminate requires <path> and optional signal or -f|--force\n", .{});
                                usageForCommandKind(.terminate);
                            },
                            .wait => {
                                err("msr2: wait requires <path>\n", .{});
                                usageForCommandKind(.wait);
                            },
                            .status => {
                                err("msr2: status requires <path>\n", .{});
                                usageForCommandKind(.status);
                            },
                            .exists => {
                                err("msr2: exists requires <path>\n", .{});
        ...
        .ok => |ok| {
            ...
            switch (owned_ok.command) {
                ...
                .current => {
                    ...
                    err("msr2: current requires a current session context (--session or MSR_SESSION)\n", .{});
                    usageCurrent();
                    return 1;
                },
                .status => |args| {
                ...
                .detach => {
                    ...
                    err("msr2: detach requires a current session context (--session or MSR_SESSION)\n", .{});
                    usageDetach();
                    return 1;
                },
                .resize => |args| {
                    var owner = openAttachmentForOwnerScopedOp(args.path, args.force) catch |e| {
                        err("msr2: resize requires current ownership or -f|--force ({s})\n", .{@errorName(e)});
                        return 1;
                    };
                    ...
                    if (existing_cli) |*cli| {
                        ...
                        _ = cli.status() catch {
                            spawnHostDetached(argv[0], path, child_argv) catch {
                                err("msr2: failed to spawn host\n", .{});
                                return 1;
                            };

                            if (!waitForReady(path, 2000)) {
                                err("msr2: session did not become ready\n", .{});
                                return 1;
                            }
                        ...
                        err("msr2: session already exists at {s}\n", .{path});
                        return 1;
                    }
