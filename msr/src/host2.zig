...
pub const SessionHost = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, opts: SpawnOptions) Error!SessionHost {
        ...
        var self = SessionHost{
            ...
            .screen_seq = 0,
            .replay_chunks = .{},
            .terminal_state = if (opts.enable_terminal_state)
                try terminal_state_vterm.VTermAdapter.init(opts.rows orelse 24, opts.cols orelse 80)
            else
                null,
        };
        self.pty = .{ .host = &self };
    ...
    pub fn deinit(self: *SessionHost) void {
        ...
        for (self.replay_chunks.items) |chunk| {
    ...
    pub fn readObservedPtyChunk(self: *SessionHost, allocator: std.mem.Allocator, max_bytes: usize) Error!?[]u8 {
        return switch (self.state) {
            .idle, .starting => Error.NotStarted,
            .running => blk: {
                const fd = self.master_fd orelse return Error.InvalidState;
                ...
                if ((pfd.revents & c.POLLIN) == 0) break :blk null;

                const capped_max = @max(@as(usize, 1), max_bytes);
                const buf = allocator.alloc(u8, capped_max) catch return Error.OutOfMemory;
                ...
                const chunk = allocator.realloc(buf, @intCast(n)) catch buf[0..@intCast(n)];
    ...
    pub fn replayAfterSeq(self: *const SessionHost, allocator: std.mem.Allocator, after_seq: u64) Error![]ReplayChunk {
        ...
        if (after_seq != 0 and self.screen_seq > after_seq and first_idx == null) {
            return Error.InvalidArgs;
        }
        if (first_idx == null) {
            return try allocator.alloc(ReplayChunk, 0);
        }
        ...
        for (self.replay_chunks.items[start_idx..], 0..) |chunk, i| {
            out[i] = .{
                .seq = chunk.seq,
                .bytes = try allocator.dupe(u8, chunk.bytes),
            };
        }
        return out;
    }
...
test "host2 wait returns child exit code" {
...
test "host2 pty write/read supports simple echo flow" {
    var host = try SessionHost.init(std.testing.allocator, .{
...
test "host2 pty read before start is not allowed" {
    var host = try SessionHost.init(std.testing.allocator, .{ .argv = &.{"/bin/sh"} });
    defer host.deinit();
    try std.testing.expectError(Error.NotStarted, host.pty.read(std.testing.allocator, 32, 10));
}

test "host2 wait after exit reuses cached status" {
...
test "host2 close before exit is invalid" {
    var host = try SessionHost.init(std.testing.allocator, .{ .argv = &.{"/bin/sh"} });
    defer host.deinit();
    try std.testing.expectError(Error.InvalidState, host.close());
}

test "host2 close after exit transitions to closed" {
...
test "host2 close is idempotent after closed" {
    var host = try SessionHost.init(std.testing.allocator, .{ .argv = &.{ "/bin/sh", "-c", "exit 0" } });
    defer host.deinit();
    try host.start();
    _ = try host.wait();
...
test "host2 readObservedPtyChunk returns a bounded chunk" {
    var host = try SessionHost.init(std.testing.allocator, .{
    ...
    while (tries < 50) : (tries += 1) {
        if (try host.readObservedPtyChunk(std.testing.allocator, 3)) |chunk| {
    ...
    try std.testing.expect(got_any);

    _ = try host.wait();
    try host.close();
}

test "host2 replayAfterSeq returns only tail after snapshot boundary" {
    ...
    var tries: usize = 0;
    while (tries < 50) : (tries += 1) {
        while (try h.readObservedPtyChunk(std.testing.allocator, 4096)) |chunk| {
