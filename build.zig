...
pub fn build(b: *std.Build) void {
    ...
    const host_session_mod = b.addModule("host_session", .{
        .root_source_file = b.path("host/src/host_session.zig"),
        .target = target,
        .optimize = optimize,
        .link_libc = true,
    });
    ...
    addVendoredLibvterm(history_test_root, b);
    const history_tests = b.addTest(.{ .root_module = history_test_root, .filters = &.{"history ownership"} });
    ...
    const stdout_actor_tests = b.addTest(.{ .root_module = stdout_actor_mod });
    const run_stdout_actor_tests = b.addRunArtifact(stdout_actor_tests);
    ...
    const vpty_input_tests = b.addTest(.{ .root_module = vpty_root, .filters = &.{"vpty input"} });
    const run_vpty_input_tests = b.addRunArtifact(vpty_input_tests);
    ...
    // Zig's testing allocator cannot detect leaked C history-event buffers.
    const history_leaks = b.addSystemCommand(&.{ "valgrind", "--leak-check=full", "--show-leak-kinds=all", "--errors-for-leak-kinds=all", "--error-exitcode=1" });
    history_leaks.addArtifactArg(history_tests);
